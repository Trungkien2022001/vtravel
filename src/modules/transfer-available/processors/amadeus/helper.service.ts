...
import { TransferProvider, TransferSession } from 'src/shared';
import { Transfer_AMADEUS_CONFIG } from 'src/shared/constants';
...
export class HelperService {
  ...
  baseSoap(acc: TransferProvider, action: string, session?: TransferSession) {
    const nonce = uuid();
  ...
  getPriceTypes(type: string, airlineCode: string) {
    switch (type) {
      ...
      case 'stopover':
        return (
          Transfer_AMADEUS_CONFIG.priceTypes[type][airlineCode] ||
          Transfer_AMADEUS_CONFIG.priceTypes[type].default
        );
      default:
        return (
          Transfer_AMADEUS_CONFIG.priceTypes.Transfer[airlineCode] ||
          Transfer_AMADEUS_CONFIG.priceTypes.Transfer.default
        );
    }
  }
  ...
  getPaxTypeMapping(airlineCode) {
    return (
      Transfer_AMADEUS_CONFIG.mapPaxType[airlineCode] ||
      Transfer_AMADEUS_CONFIG.mapPaxType.default
    );
  }

  getRePaxtypeMapping(airlineCode) {
    return (
      Transfer_AMADEUS_CONFIG.reMapPaxType[airlineCode] ||
      Transfer_AMADEUS_CONFIG.reMapPaxType.default
    );
  }

  searchByCompanyIdentity(acc: TransferProvider) {
    return (
      acc.airline_code &&
      acc.custom_config &&
