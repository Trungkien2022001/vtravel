...
import { buildTourDetailCacheKey, AppError } from 'src/common';
...
export class TourDetailService {
  ...
  async getTourDetail(body: TourDetailDto) {
  ...
  async checkAvailableTour(body: TourDetailDto) {
  ...
  async findAllRate(body: TourDetailDto) {
    const redisKey = buildTourDetailCacheKey(body);
    const numOfPax = body.adult + body.children + body.infant;
    const handle = async () => {
